...
function App() {
  ...
  // Esta peticion se realiza primero, devuelve la geolocalizacion del usuario   // Al ser una peticion (consumo de API) que pediremos 1 vez usamos este hook para no tener un bucle infinito / Esto es la 1era peticion para obtener las coordenadas
  useEffect(() => {
    setisLoading(true) 
    navigator.geolocation.getCurrentPosition(success)
  }, [] )
  
  // Creamos un segundo useEffect porque para el 1er renderizado no tendriamos los valores de 'coords' (1er useEffect) , cuando cambie el estado de coords es que se ejecutará el useEffect / Esta 2da peticion nos da la informacion de todo el clima segun las coordenadas
  useEffect(() => {
    if (coords){
